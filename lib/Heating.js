...
class Heating extends EventEmitter {
  constructor(homey, log) {
    ...
    this._lastProfile = null;          // in-memory: what this process last applied
    this._lastApply = 0;
  }

  _announced() { return this.homey.settings.get('heatingLastProfile') || null; }
  _setAnnounced(p) { this.homey.settings.set('heatingLastProfile', p); }
  ...
  /** Put one room on a profile's temperature as a manual hold until the next switch point. */
  async setRoomProfile(serial, profile) {
    const cfg = this.get();
    const target = this.targetFor(cfg, profile, serial);
    if (target == null) throw new Error('Unknown profile');
    const dev = this._thermostats().find((d) => d.getData().serial === serial);
    if (!dev) throw new Error('Unknown thermostat');
    this._expected.delete(serial);
    await dev._writeSetpoint(target);
    this.noteSetpointChange(serial, target);
    return target;
  }
  ...
  /* ---------- engine ---------- */
  _thermostats() {
    try { return this.homey.drivers.getDriver('thermostat').getDevices(); } catch { return []; }
  }
  ...
  async tick() {
    ...
    if (changed) {
      ...
      // Only announce a real transition — not the first tick after an app restart.
      const before = this._lastProfile != null ? this._lastProfile : this._announced();
      if (before !== profile) this.emit('profile-changed', { profile, previous: before });
      this._setAnnounced(profile);
