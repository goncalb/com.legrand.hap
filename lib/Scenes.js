  ...
  async run(id) {
    ...
    const results = await Promise.allSettled(scene.entries.map(async (e) => {
      ...
      if (driverId === 'shutter') {
        ...
        if (e.mode === 'off') await dev.hap.setChar(dev.serial, 'Active', 0);
        else {
          if (dev.acc().chars.TargetHeaterCoolerState) await dev.hap.setChar(dev.serial, 'TargetHeaterCoolerState', 1);
          await dev.hap.setChar(dev.serial, 'Active', 1);
          if (e.target != null) await dev.hap.setChar(dev.serial, 'HeatingThresholdTemperature', Number(e.target));
        }
      }
    }));
