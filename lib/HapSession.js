...
class HapEndpoint extends EventEmitter {
  ...
  onService(svc) {
    const changed = !this.cfg.address || this.cfg.address.address !== svc.address || this.cfg.address.port !== svc.port;
    if (changed) { this.cfg.address = { address: svc.address, port: svc.port }; this.session._persist(); }
  ...
  _onDisconnect(why) {
    ...
    this.connected = false;
    if (this.client) try { this.client.close(); } catch (e) { /* noop */ }
    this.client = null;
  ...
  _parseAccessory(raw) {
    ...
    for (const svc of raw.services) {
      ...
      if (st === SERVICE.Lightbulb) { acc.klass = 'light'; keep('On'); keep('Brightness'); }
    ...
    acc.shape = { name: acc.name, model: acc.model, klass: acc.klass,
  ...
  _onEvent(ev) {
    ...
    for (const item of ev.characteristics) {
      ...
      if (btn) { this.session.log('info', serial, `Button ${btn.index} pressed`); this.session.emit('char', { serial, type: 'ButtonEvent', value: btn.index }); continue; }
...
class HapSession extends EventEmitter {
  ...
  async stop() {
    ...
    if (this._discovery) try { this._discovery.stop(); } catch (e) { /* noop */ }
  ...
  async pair(id, pin, address) {
    ...
    const target = svc ? { address: svc.address, port: svc.port } : address;
    ...
    const client = new HttpClient(id, target.address, target.port || 5001);
    await client.pairSetup(formatted);
    const pairing = client.getLongTermData();
    try { client.close(); } catch (e) { /* noop */ }
    return this.addEndpoint(id, { pairing, address: { address: target.address, port: target.port || 5001 }, name: svc ? svc.name : null });
  }
  ...
  async readChar(serial, charName) {
    const { acc, ep } = this._epFor(serial);
    const ch = acc.chars[charName];
    if (!ch) return null;
    const res = await ep.client.getCharacteristics([`${acc.aid}.${ch.iid}`]);
    const item = res.characteristics && res.characteristics[0];
    if (item && item.value !== undefined && item.value !== null) ch.value = item.value;
    return ch.value;
  }
